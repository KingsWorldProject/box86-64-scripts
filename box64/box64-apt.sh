...
# box64-apt.sh: script to compile box64 to a deb and write it to the apt repository
...
function error() {
  ...
  mv /home/pi/Documents/box64-auto-build/commit.txt.bak /home/pi/Documents/box64-auto-build/commit.txt
	cat /home/pi/Documents/box64-auto-build/box64.log | mail -s "Box64 Build Failed" $MAILTO
     	exit 1
 	break
}
...
# Box64 lets you run x86_64 Linux programs (such as games) on non-x86_64 Linux systems, like ARM (host system needs to be 64bit little-endian)" > DEBIAN/control
...
cp -r $DEBDIR/box64_$NOWDAY/*.deb /home/pi/Documents/box64-debs/debian/ || error "Failed to copy to debian/ folder"
...
git commit -m "Updated Box64 v$BOX64VER to $BOX64COMMIT" || error "Failed to run git commit"
